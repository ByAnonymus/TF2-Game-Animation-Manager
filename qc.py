...
class BYANON_OT_anim_port(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for b in buffer:            
            print(b)            
            if (self.suffix_enum.lower()+"_" in b.lower() or self.suffix_enum.lower()+"\"" in b.lower() ) and "@" not in b and "layer" not in b and "$sequence" in b and "aim" not in b and "run" not in b:
                b1 = buffer.index(b)         
